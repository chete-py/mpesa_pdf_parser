...
highest = highest_payment['Amount']


highest_month_row = months[months['Amount'] == months['Amount'].max()]
highest_month = highest_month_row['Month']
highest_month_amount = highest_month_row['Amount']

highest_day_row = day[day['Amount'] == day['Amount'].max()]
highest_day = highest_day_row['Day']
highest_day_amount = highest_day_row['Amount']
...
with card_container(key='global'):
    ...
    with tab1:
        with card_container(key='dashboard'):
            cols = st.columns(3)
            with cols[0]:
                ui.metric_card(title="Highest Payment", content=f'Ksh. {highest}',  key="card1")
            with cols[1]:
